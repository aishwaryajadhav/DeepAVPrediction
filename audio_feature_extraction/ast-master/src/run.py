...
parser.add_argument("--dataset", type=str, default="audioset", help="the dataset used", choices=["dav","audioset", "esc50", "speechcommands"])
...
if args.model == 'ast':
    ...
    norm_stats = {'dav':[-3.8440912, 4.1375813], 'audioset':[-4.2677393, 4.5689974], 'esc50':[-6.6268077, 5.358466], 'speechcommands':[-6.845978, 5.5654526]}
    target_length = {'dav':512, 'audioset':1024, 'esc50':512, 'speechcommands':128}
    # if add noise for data augmentation, only use for speech commands
    noise = {'dav': False, 'audioset': False, 'esc50': False, 'speechcommands':True}
    ...
    ast_mdl = models.ASTModel(label_dim=args.n_class, fstride=args.fstride, tstride=args.tstride, input_fdim=128,
                                  input_tdim=target_length[args.dataset], imagenet_pretrain=args.imagenet_pretrain,
                                  audioset_pretrain=args.audioset_pretrain, model_size='base384')
    checkpoint = torch.load('../../../efs/exp/test-dav-f10-t10-impTrue-aspTrue-b16-lr1e-5-ensemble-mixup/models/best_audio_model.pth', map_location='cuda')
