...
def train(audio_model, train_loader, test_loader, args):
    ...
    optimizer = torch.optim.Adam(trainables, 1.5e-06, weight_decay=5e-7, betas=(0.95, 0.999))
    optimizer.load_state_dict(torch.load('../../../efs/exp/test-dav-f10-t10-impTrue-aspTrue-b16-lr1e-5-ensemble-mixup/models/best_optim_state.pth'))
    ...
    if args.dataset == 'audioset':
        ...
        loss_fn = nn.BCEWithLogitsLoss()
        warmup = False
    elif args.dataset == 'dav':
        print('scheduler for esc-50/dav is used')
        # scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, list(range(10,41)), gamma=0.85)
    ...
    while epoch < args.n_epochs + 1:
        ...
        cum_mAP = np.mean([stat['AP'] for stat in cum_stats if not math.isnan(stat['AP'])])
        cum_mAUC = np.mean([stat['auc'] for stat in cum_stats])
        cum_acc = cum_stats[0]['acc']
        # print([stat['AP'] for stat in stats])
        mAP = np.mean([stat['AP'] for stat in stats if not math.isnan(stat['AP'])])
        ...
        if main_metrics == 'mAP':
            print("mAP: {:.6f}".format(cum_mAP))
        else:
            print("acc: {:.6f}".format(cum_acc))
        print("AUC: {:.6f}".format(cum_mAUC))
        ...
        if best_cum_epoch==epoch:
            torch.save(audio_model.state_dict(), "%s/models/best_weight_avg_audio_model.pth" % (exp_dir))
            torch.save(optimizer.state_dict(), "%s/models/best_optim_state.pth" % (exp_dir))
    ...
    if args.dataset == 'audioset' or args.dataset == 'dav':
        if len(train_loader.dataset) > 2e5:
