...
class StaticImageDemo:
    ...
    def __call__(self, image,audio):
        predictions = self.predictor(image,audio)
        return predictions
...
class StaticImagePredictor:
    ...
    def __call__(self, input_image, audio):
        ...
        with torch.no_grad():
            ...
            input = {'image': image, 'height': height, 'width': width,'audio':audio}
...
def process_audio(audio_features):
        d={}
        for idx,f_name in enumerate(audio_features['audio_name']):
            vid_name=f_name.split('.')[0]
            t={}
            t['conv_1']=audio_features['conv_1'][idx]
            t['conv_2']=audio_features['conv_2'][idx]
            t['conv_3']=audio_features['conv_3'][idx]
            t['conv_4']=audio_features['conv_4'][idx]
            t['conv_5']=audio_features['conv_5'][idx]
            t['conv_6']=audio_features['conv_6'][idx]
            t['pooling_output']=audio_features['pooling_output'][idx]
            d[vid_name]=t
        return d
...
if __name__ == '__main__':
    ...
    for video in tqdm(data['videos']):
        vid_id = video['file_names'][0].split('/')[0]
        if vid_id not in audio_files:
            continue
        memory = dict()
        images = [
            read_image(osp.join('datasets/youtubevis/train', image_name),
                       'BGR') for image_name in video['file_names']
        ]
        video_name = video['file_names'][0].split('/')[0]

        for index in range(len(images)):
            predictions = demo(images[index],audio_features[video_name])[0]['instances']
